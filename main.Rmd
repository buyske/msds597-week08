...
x <- separate(x, x, into = c("average" , "Bound") , sep = "\\[" , fill = "right")
...
##Kiyoon_Function_Fixed

```{r function}

fix_who_column_fixed <- function(x){
...
df <- read.csv("data/data.csv", skip=2)

colnames(df)[3] <- "Violence"

fix_who_column_fixed(df$violence)
